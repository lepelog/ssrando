...
use crate::LINK_PTR;
use crate::{console_print, rvl_mem::ios_allocate, rvl_mutex::WiiMutex};
...
extern "C" fn on_recv(result: c_int, usr_data: *mut c_void) {
    ...
    if result_data.starts_with(b"pos") && unsafe { !LINK_PTR.is_null() } {
    ...
    if result_data.starts_with(b"fly") && unsafe { !LINK_PTR.is_null() } {
        unsafe { (*LINK_PTR).pos_y += 200f32 };
