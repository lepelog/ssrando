...
ITEM_TEXT_MAPPING = {
    "Slingshot": "Slingshit",
    "Bomb Bag": "BiT Machine Bag",
    "Clawshots": "Hookshot",
    "Gust Bellows": "Kina Blower",
    "Bow": "Diplomacy",
    "Faron Song of the Hero Part": "Song of the Fabulous",
    "Eldin Song of the Hero Part": "Song of the One",
    "Lanayru Song of the Hero Part": "Song of the Only",
    "Ballad of the Goddess": "Salad of the Goddess",
    "Gratitude Crystal Pack": "Channel Points",
    "Water Scale": "Drowning Scale",
    "Cursed Medal": "Peatrice Medal",
    "Wooden Shield": "Pot Lid",
    "Hylian Shield": "Botbegone's Protection",
    "Whip": "Panda-torturing tool",
    "Bug Net": "Jasnix%",
    "Cawlin's Letter": "Cawlin's Toilet Paper",
    "Horned Colossus Beetle": "Beedle's Beetle",
    "Tumbleweed": "Validationweed",
}
...
class LocationGossipStoneHint(GossipStoneHint):
    ...
    def to_gossip_stone_text(self) -> List[str]:
        ...
        itemname = ITEM_TEXT_MAPPING.get(self.item, self.item)
        return [f"<r<{zone} - {specific_loc}>> has <y<{itemname}>>"]

    def to_spoiler_log_text(self) -> str:
        itemname = ITEM_TEXT_MAPPING.get(self.item, self.item)
        return f"{self.location_name} has {itemname}"
...
class ItemGossipStoneHint(GossipStoneHint):
    ...
    def to_gossip_stone_text(self) -> List[str]:
        zone, specific_loc = Logic.split_location_name_by_zone(self.location_name)
        itemname = ITEM_TEXT_MAPPING.get(self.item, self.item)
        return [f"<y<{itemname}>> can be found at <r<{zone}: {specific_loc}>>"]

    def to_spoiler_log_text(self) -> str:
        itemname = ITEM_TEXT_MAPPING.get(self.item, self.item)
        return f"{itemname} is on {self.location_name}"
